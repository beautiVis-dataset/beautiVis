...
def process_csv_file(csv_path):
    ...
    match = re.match(r"(\d{4}-\d{2})-visualizations\.csv", basename)
    if not match:
        print(f"Filename {basename} does not match expected pattern. Skipping.")
        return
    year_month = match.group(1)
    
    output_dir = os.path.join("vis_images", year_month)
    os.makedirs(output_dir, exist_ok=True)

    with open(csv_path, "r", newline='', encoding="utf-8") as infile:
        reader = csv.DictReader(infile)
        for row in reader:
            reddit_url = row.get("json_full_permalink")
            title = row.get("json_title", "no title")
            if reddit_url:
                print(f"\nProcessing: {title}")
                image_url = get_image_url_from_json(reddit_url)
                print(f"Extracted image URL for '{title}': {image_url}")
                if image_url:
                    filename = row.get("pp_image_file")
                    if not filename:
                ...
                print("No json_full_permalink found in row.")

def main():
    csv_files = glob.glob(os.path.join("vis_csv", "*-visualizations.csv"))
    if not csv_files:
        print("No CSV files found in the vis_csv folder.")
        return
